...
public class FinancialFunctions {
	...
	public static double averageOverX(int seconds, int start,
			String[] homeMarket) {
		double average = 0.0;
		if ((start - seconds) >= 0) {
	...
	public static double rateOfChangeOverX(int seconds, int start) {
		...
		if ((start - seconds) >= 0) {
			double endPrice = Double
					.parseDouble(MultiValuedRegression.bitStampRecords[start]);
	...
	public static double relativeStrength(int seconds, int start) {
		...
		if ((start + seconds) <= MultiValuedRegression.bitstampNumOfRecords
				&& (start - 1) >= 0) {
			for (int i = 0; i < seconds; i++) {
				double currentSecond = Double
						.parseDouble(MultiValuedRegression.bitStampRecords[start
								+ i]);
				double previousSecond = Double
						.parseDouble(MultiValuedRegression.bitStampRecords[start
								+ i - 1]);
	...
	// moving average
	public static double exponentialMovingAverage(int seconds, int start,
			String[] homeMarket) {
		double ema = 0.0;
		if ((start - seconds) >= 0) {
			double secondsAverage = averageOverX(seconds, start - seconds,
					homeMarket);
			double currentClosingPrice = Double.parseDouble(homeMarket[start]);
			ema = currentClosingPrice * (2 / (seconds + 1)) + secondsAverage
					* (1 - (2 / (seconds + 1)));
		}
	...
	public static double MACD(int start, String[] homeMarket) {
		double macd = exponentialMovingAverage(12, start, homeMarket)
				- exponentialMovingAverage(26, start, homeMarket);
		return macd;
	}
	...
	public static double maxValueOverX(int seconds, int start) {
		...
		if ((start - seconds) >= 0) {
			for (int i = start; i >= (start - seconds); i--) {
				if (Double
						.parseDouble(MultiValuedRegression.bitStampRecords[i]) > max) {
					max = Double
							.parseDouble(MultiValuedRegression.bitStampRecords[i]);
				}
			}
		}
		return max;
	...
	public static double minValueOverX(int seconds, int start) {
		...
		if ((start - seconds) >= 0) {
			for (int i = start; i >= (start - seconds); i--) {
				if (Double
						.parseDouble(MultiValuedRegression.bitStampRecords[i]) < min) {
					min = Double
							.parseDouble(MultiValuedRegression.bitStampRecords[i]);
				}
			}
	...
	public static double averageMarketVariance(String[] homeMarket,
			String[] otherMarket) {
		...
		for (int i = 0; i < numberOfRecords; i++) {
			differenceSum += Math.abs(Double.parseDouble(homeMarket[i])
					- Double.parseDouble(otherMarket[i]));
		}
	...
	public static double currentVarianceValue(String[] homeMarket,
			String[] otherMarket, double averageDiff, int start, int seconds) {
		// double overAllAverage = averageMarketVariance(homeMarket,
		// otherMarket); //Move this to multivalued regression and only
		// calculate once
		// double averageHome = averageOverX(seconds, start, homeMarket);
		// double averageAway = averageOverX(seconds, start, otherMarket);
		double difference = Double.parseDouble(otherMarket[start])
				- Double.parseDouble(homeMarket[start]);
		double toReturn = 0.0;
		...
		if ((start - seconds) >= 0) {
			...
			for (int i = start; i >= start - seconds; i--) {
				sumOfSquares += Math.pow(
						(Double.parseDouble(homeMarket[i]) - mean), 2);
			}
	...
	public static String[] buildRecordsArray(String source) {
		...
		try {
			...
			while ((line = br.readLine()) != null
					&& i < MultiValuedRegression.bitstampNumOfRecords) {
				toReturn[i] = line;
				i++;
			}
		} catch (IOException e) {
	...
	public static int buyHoldSell(String[] homeMarket, int start) {
		...
		if ((start + 35) < MultiValuedRegression.bitstampNumOfRecords) {
			...
			if ((priceIn5Min - currentPrice) > calcFee(currentPrice, 0.005)) {
				return 1;
			} else if ((priceIn5Min - currentPrice) < calcFee(currentPrice,
					0.005)) {
				return -1;
			} else {
				return 0;
